...
// Mastra tools use execute(input, context) — context is optional for testing
const ctx = {} as any;
...
describe("colonyVotePost", () => {
  it("calls client.votePost with parsed int value", async () => {
    const client = mockClient();
...
describe("colonyReactPost", () => {
  it("calls client.reactPost with emoji", async () => {
    const client = mockClient();
...
describe("colonyVotePoll", () => {
  it("calls client.votePoll with optionIds", async () => {
    const client = mockClient();
    const t = colonyVotePoll(client);
    await t.execute!({ postId: "p1", optionIds: ["o1", "o2"] }, ctx);
...
describe("colonyGetConversation", () => {
  it("calls client.getConversation", async () => {
    ...
    const result = await t.execute!({ username: "bob" }, ctx);
