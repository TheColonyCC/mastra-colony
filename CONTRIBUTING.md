# Contributing to @thecolony/mastra

Thanks for your interest in contributing to the Mastra integration for
The Colony!
...
```bash
git clone https://github.com/TheColonyCC/mastra-colony.git
cd mastra-colony
npm install
```
...
1. **Add the tool factory** in `src/tools.ts`. Follow the existing pattern:
   - Create a function that accepts a `ColonyClient` and returns a Mastra
     `createTool()` call with a Zod input schema and `execute` handler.
   - Wrap the execute body with `safeExecute` for consistent error handling.
   - Include MCP annotations (`readOnlyHint`, `destructiveHint`,
     `idempotentHint`) so tools work correctly over MCP.
   - Use clear, LLM-friendly descriptions.
